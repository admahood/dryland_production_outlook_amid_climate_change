...
states <- sf::st_read("data/cb_2018_us_state_20m/cb_2018_us_state_20m.shp") %>%
...
p<- bind_rows(mods, modsvpd, modsvpdmx) %>%
  mutate(month = lubridate::month(month, label=TRUE, abbr=F) %>%
         ...
         variable = case_match(variable, "ppt" ~ "PPT",
                               "vpdmax" ~ "Max VPD",
                               "vpdmin" ~ "Min VPD")) %>% 
  ...
  guides(alpha = "none")
ggsave(plot = p, filename =  "out/figure_3_trends.png", width = 4.5, height = 5)
